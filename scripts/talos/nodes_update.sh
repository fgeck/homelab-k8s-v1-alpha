...
log_exec talosctl apply-config --nodes "$FINAL_CONTROL_PLANE_IP" --file "secrets/talos/controlplane.yaml"
log_exec talosctl apply-config --nodes "$FINAL_WORKER_1_IP" --file "secrets/talos/worker-1.yaml"
log_exec talosctl apply-config --nodes "$FINAL_WORKER_2_IP" --file "secrets/talos/worker-2.yaml"
